...
//-----------------------------------------------------------------------------
// Title         : Zedboard TCP Endpoint Demonstrator
//-----------------------------------------------------------------------------
// File          : zedboard_tcp_endpoint.sv
// Author        : Christophe Clienti <cclienti@wavecruncher.net>
// Created       : 2026-08-29
// Last modified : 2026-09-03
//-----------------------------------------------------------------------------
// Description: Board demonstrator of rmii_eth_tcp_endpoint on a Zedboard
...
// answers arping and ping, and echoes a TCP connection to port 23,
// so `telnet 192.168.90.42` or `nc 192.168.90.42 23` types back what
// you send. The echo loopback -- m_app straight back to s_app -- is
// wired here in the board wrapper, not inside the endpoint.
//
...
module zedboard_tcp_endpoint (
    ...
    localparam logic [15:0] LISTEN    = 16'd23;   // telnet
...
`endif
    ...
    // Echo loopback: the socket's received stream straight back to its
    // send stream, the close token included
    logic [7:0] app_tdata;
    logic       app_tuser, app_tvalid, app_tlast, app_tready;

    rmii_eth_tcp_endpoint
    #(
        .LOG2_FIFO_DEPTH   (11),
        .LOG2_ICMP_DEPTH   (11),
        .LOG2_TCP_RX_DEPTH (11),
        .LOG2_TCP_TX_DEPTH (11),
        .TCP_MSS           (1460)
    )
    rmii_eth_tcp_endpoint_inst
    (
        .clock         (refclk),
        .sreset        (sreset),
        .local_mac     (LOCAL_MAC),
