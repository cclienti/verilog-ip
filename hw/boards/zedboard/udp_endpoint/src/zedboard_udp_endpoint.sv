...
//-----------------------------------------------------------------------------
// Title         : Zedboard UDP Endpoint Demonstrator
//-----------------------------------------------------------------------------
// File          : zedboard_udp_endpoint.sv
// Author        : Christophe Clienti <cclienti@wavecruncher.net>
// Created       : 2026-09-15
// Last modified : 2026-09-15
//-----------------------------------------------------------------------------
// Description: Board demonstrator of rmii_eth_udp_endpoint on a Zedboard
...
// the module's CLKIN pin. Identity is fixed here: 192.168.90.42
// answers arping and ping, and echoes UDP datagrams to port 7 (the
// classic echo port), so `nc -u 192.168.90.42 7` types back what you
// send. The echo loopback -- m_app straight back to s_app, address
// fields included -- is wired here in the board wrapper, not inside
// the endpoint.
//
// Clock geometry, the one thing this wrapper exists for -- identical
// to the TCP demonstrator's, whose README is the reference: the
// forwarded clock is a copy of the fabric clock through an ODDR,
// transmit pins registered on the falling edge, receive on the rising
// edge, both pushed into the IOBs. Nothing about it changes here.
//
...
`endif
    ...
    // Echo loopback: the socket's received datagram stream straight
    // back to its send stream, the sender's address riding along so
    // the reply goes back to whoever sent it
    logic [7:0]  app_tdata;
    logic        app_tvalid, app_tlast, app_tready;
    logic [47:0] app_peer_mac;
    logic [31:0] app_peer_ip;
    logic [15:0] app_peer_port;

    rmii_eth_udp_endpoint
    #(
        .LOG2_FIFO_DEPTH   (11),
        .LOG2_ICMP_DEPTH   (11),
        .LOG2_UDP_RX_DEPTH (11),
        .LOG2_UDP_TX_DEPTH (11)
    )
    rmii_eth_udp_endpoint_inst
    (
        .clock            (refclk),
        .sreset           (sreset),
        .local_mac        (LOCAL_MAC),
        ...
        .m_app_peer_mac   (app_peer_mac),
        .m_app_peer_ip    (app_peer_ip),
        .m_app_peer_port  (app_peer_port),
        .s_app_tdata      (app_tdata),
        .s_app_tvalid     (app_tvalid),
        .s_app_tlast      (app_tlast),
        .s_app_tready     (app_tready),
        .s_app_dst_mac    (app_peer_mac),
        .s_app_dst_ip     (app_peer_ip),
        .s_app_dst_port   (app_peer_port),
        .phy_rxd          (rxd_q),
        .phy_crs_dv       (crs_dv_q),
        .phy_txd          (ep_txd),
        .phy_txen         (ep_txen),
