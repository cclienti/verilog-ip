...
//-----------------------------------------------------------------------------
// Title         : Zedboard UDP Endpoint Demonstrator Testbench
//-----------------------------------------------------------------------------
// File          : zedboard_udp_endpoint_tb.sv
// Author        : Christophe Clienti <cclienti@wavecruncher.net>
// Created       : 2026-09-15
// Last modified : 2026-09-15
//-----------------------------------------------------------------------------
// Description :
// Smoke test of the board wrapper, identical in scope and structure
// to the TCP demonstrator's own -- the endpoint chain is verified in
// depth by rmii_eth_udp_endpoint_tb, so this bench checks what the
// wrapper adds, and none of it is UDP-specific. The only clock driven
// here is the board's 100 MHz oscillator -- everything on the PHY
// side of the bench is timed by phy_clkin, the reference the wrapper
// generates and forwards, which is what the module would use. So the
// bench also stands in for the PHY's timing: it launches receive
// dibits a clock-to-out after the forwarded rising edge, samples the
// transmit pins on that same edge, and fails if they move inside the
// LAN8720A's 4 ns setup / 2 ns hold window -- which is what a
// transmit launch put back on the rising edge would do.
//
...
module zedboard_udp_endpoint_tb;
    ...
    zedboard_udp_endpoint zedboard_eth_endpoint_inst
    (
        .clk100     (clk100),
        .btn_reset  (btn),
        .phy_clkin  (clkin),
    ...
    initial begin
        ...
        if (errors == 0)
          $display("zedboard_udp_endpoint_tb: ALL TESTS PASSED");
        else
          $display("zedboard_udp_endpoint_tb: %0d ERROR(S)", errors);
        $finish;
    end
