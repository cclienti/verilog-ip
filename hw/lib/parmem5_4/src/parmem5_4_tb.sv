...
// Self-checking testbench for parmem5_4 (5 banks, 4 lanes).
// Phases:
//   1. bijection: single-lane fill, single-lane read back
//   2. group read, signed strides with stride % 5 != 0: no conflict,
...
module parmem5_4_tb();
   ...
   parmem5_4 #(.DEPTH(DEPTH), .WIDTH(WIDTH), .STRIDE_W(STRIDE_W),
               .OUTREGA(0), .OUTREGB(0))
   parmem5_4_inst (.clka(clka), .en(en), .wen(wen), .lane_en(lane_en),
   ...
   parmem5_4_adr_inst (.clka(clka), .en(en_p), .wen(wen_p),
   ...
   //----------------------------------------------------------------
   initial begin
      $dumpfile("parmem5_4_tb.vcd");
      $dumpvars(0, parmem5_4_tb);
   end
   ...
   initial begin
      ...
      //--- Phase 2: group read, signed strides, stride %% 5 != 0 ------
      ...
      $display("phase 2 (group read, stride %% 5 != 0) done");
      ...
      // quad read through the pipeline, stride % 5 != 0
      @(negedge clka);
      en_p = 1; wen_p = 0; lane_en_p = '1;
      addr_p = 6; stride_p = STRIDE_W'(2);
      #2;
      check("adrreg quad: no conflict", conflict_p === 1'b0);
      check("adrreg quad: no oob", oob_p === '0);
      @(negedge clka);
      idle_p();
      @(negedge clka);
      for (int i = 0; i < NB_LANES; i++) begin
         check($sformatf("adrreg quad: lane %0d data", i),
      ...
      //--- Summary ------------------------------------------------------
      if (errors == 0) begin
         $display("parmem5_4_tb: ALL TESTS PASSED (%0d words)", WORDS);
      end
      else begin
         $display("parmem5_4_tb: FAILED with %0d error(s)", errors);
      end
      $finish;
   end


endmodule // parmem5_4_tb
