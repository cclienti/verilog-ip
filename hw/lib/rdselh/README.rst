Half-Word Read Select
======================

Description
-----------

The ``rdselh`` module selects a half-word (16 bits) from a 32-bit input word (``in``) according to
the ``sel`` input and extends it to 32 bits. If ``is_signed`` is set, the output is sign-extended;
otherwise, it is zero-extended. This is useful for half-word extraction and extension in bus or
memory interfaces.
...
sel         input wire    1         Half-word select (0=LSB, 1=MSB)
