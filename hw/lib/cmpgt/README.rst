...
The ``cmpgt`` module is a dynamically signed or unsigned "strictly greater than" comparator. It
compares two input words (``a``, ``b``) and outputs ``1`` if ``a`` is strictly greater than ``b``,
otherwise outputs ``0``. The comparison is performed as signed if ``is_signed`` is set, or as
unsigned otherwise. By complementing the output, it can function as a "less or equal" comparator.
