...
// Self-checking testbench for parmem17_16 (17 banks, 16 lanes).
// Phases:
//   1. bijection: single-lane fill, single-lane read back
//   2. group read, signed strides with stride % 17 != 0: no conflict,
//      every active lane correct
//   3. stride % 17 == 0 (including 0): conflict, lane 0 wins
//   4. group write, verified by read-back
...
module parmem17_16_tb();

   parameter DEPTH    = 4;
   parameter WIDTH    = 32;
   parameter STRIDE_W = 6;

   localparam NB_BANKS = 17;
   localparam NB_LANES = 16;
   localparam AW       = DEPTH + 5;
   ...
   parmem17_16 #(.DEPTH(DEPTH), .WIDTH(WIDTH), .STRIDE_W(STRIDE_W),
                .OUTREGA(0), .OUTREGB(0))
   parmem17_16_inst (.clka(clka), .en(en), .wen(wen), .lane_en(lane_en),
   ...
   parmem17_16_adr_inst (.clka(clka), .en(en_p), .wen(wen_p),
   ...
   //----------------------------------------------------------------
   initial begin
      $dumpfile("parmem17_16_tb.vcd");
      $dumpvars(0, parmem17_16_tb);
   end
   ...
   initial begin
      ...
      //--- Phase 2: group read, signed strides, stride %% M != 0 ------
      ...
      $display("phase 2 (group read, stride %% %0d != 0) done", NB_BANKS);

      //--- Phase 3: stride %% M == 0 -> conflict, lane 0 wins ---------
      ...
      $display("phase 3 (stride %% %0d == 0, conflicts) done", NB_BANKS);

      //--- Phase 4: group write, verified by read-back ----------------
      ...
      $display("phase 4 (group write) done");

      //--- Phase 5: partial lane masks ---------------------------------
      begin
         // disabled lane must have no side effect on a group write
         @(negedge clka);
         ...
         // single active lane, stride % M == 0: no pair -> no conflict
         @(negedge clka);
      ...
      // last lane overflows, lower lanes proceed
      begin
         a0 = WORDS - 1 - (NB_LANES - 2);
         @(negedge clka);
         en = 1; wen = 0; lane_en = '1;
         addr = a0[AW-1:0]; stride = STRIDE_W'(1);
         #2;
         check("oob: last lane flagged", oob[NB_LANES-1] === 1'b1);
         check("oob: lower lanes clean",
               oob[NB_LANES-2:0] === '0);
         @(negedge clka);
         idle();
      ...
      // Most-negative-plus-one stride: EA1 < 0 from addr 0, truncating
      // into a valid low index
      begin
         s_neg = 1 - (1 << (STRIDE_W - 1));
         @(negedge clka);
         en = 1; wen = 1; lane_en = {{(NB_LANES-2){1'b0}}, 2'b11};
      ...
      // group read through the pipeline, stride % M != 0
      @(negedge clka);
      en_p = 1; wen_p = 0; lane_en_p = '1;
      addr_p = 6; stride_p = STRIDE_W'(2);
      #2;
      check("adrreg group: no conflict", conflict_p === 1'b0);
      check("adrreg group: no oob", oob_p === '0);
      @(negedge clka);
      idle_p();
      @(negedge clka);
      for (int i = 0; i < NB_LANES; i++) begin
         check($sformatf("adrreg group: lane %0d data", i),
               doa_p[i*WIDTH +: WIDTH] === (pattern(6 + 2 * i) ^ 32'h3C3C3C3C));
      end
      ...
      //--- Summary ------------------------------------------------------
      if (errors == 0) begin
         $display("parmem17_16_tb: ALL TESTS PASSED (%0d words)", WORDS);
      end
      else begin
         $display("parmem17_16_tb: FAILED with %0d error(s)", errors);
      end
      $finish;
   end


endmodule // parmem17_16_tb
