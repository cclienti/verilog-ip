...
// Parallel memory: 17 prime-interleaved banks, 16-lane strided access
...
// counterfactual and for pipelined/faster-fabric exploration (ADRREG).
//
//   lane i (i = 0..15): EA_i = addr + i*stride, enabled by lane_en[i];
//   all enabled lanes share `wen` (group load or group store).
//
// CRT addressing, no divider: bank = EA mod 17, index = EA[DEPTH-1:0]
// (bijective since gcd(17, 2^DEPTH) = 1). The residue tree folds eight
// nibbles with alternating signs (16 == -1 mod 17, casting out
// elevens). Lanes collide iff stride == 0 (mod 17) -- `conflict` is
// ONE bit and a pure function of the stride residue and the lane mask;
// it may assert together with oob*, the trap taking precedence. On
...
module parmem17_16
  #(parameter DEPTH    = 10,  //log2 of words per bank; 17*2^DEPTH words total
    parameter WIDTH    = 32,
    parameter STRIDE_W = 12,  //signed stride width, in words; <= DEPTH+5
   ...
   (//Side A: one 16-lane strided load/store access group
    input  logic                clka,
    input  logic                en,
    input  logic                wen,      //shared by the group
    input  logic [15:0]         lane_en,  //per-lane enable
    input  logic [DEPTH+4:0]    addr,     //lane 0 linear address
    input  logic [STRIDE_W-1:0] stride,   //signed, in words
    input  logic [16*WIDTH-1:0] dia,      //lane write data
    output logic [16*WIDTH-1:0] doa,      //lane read data

    output logic                conflict, //stride % 17 == 0: serialize
    output logic [15:0]         oob,      //EA_i out of range

    //Side B: single linear-addressed port (network interface)
    input  logic                clkb,
    input  logic                enb,
    input  logic                web,
    input  logic [DEPTH+4:0]    addrb,
    input  logic [WIDTH-1:0]    dib,
    output logic [WIDTH-1:0]    dob,
    output logic                oobb);

   localparam NB  = 17;                   //banks
   localparam NL  = 16;                   //lanes
   localparam CLM = 5;                    //bank id width
   localparam AW  = DEPTH + 5;            //linear address width
   localparam EAW = AW + 6;               //lane EA: sign + |15*stride| margin
   localparam SMW = STRIDE_W + 5;         //lane stride multiple, i <= 15

   // mod-17 digit fold: eight nibbles with alternating signs
   // (16 == -1 mod 17, the BSP fold); |neg| <= 4*15 = 60 -> OFFSET =
   // 68 (4*17); first-fold value <= 60 + 68 = 128 (8 bits)
   localparam SRW    = 8;
   localparam OFFSET = 68;
   // 2^STRIDE_W mod 17 (two's-complement sign correction; 2^k mod 17
   // cycles with period 8: 1,2,4,8,16,15,13,9)
   localparam K8 = STRIDE_W % 8;
   localparam CM = (K8 == 0) ? 1 : (K8 == 1) ? 2 : (K8 == 2) ? 4
                 : (K8 == 3) ? 8 : (K8 == 4) ? 16 : (K8 == 5) ? 15
                 : (K8 == 6) ? 13 : 9;

   initial begin
      assert (STRIDE_W <= AW)
        else $fatal(1, "STRIDE_W must be <= DEPTH + 5");
   end


   //----------------------------------------------------------------
   // mod-17 helpers (all arithmetic on narrow bounded vectors)
   //----------------------------------------------------------------
   function automatic logic [CLM-1:0] mod17(input logic [31:0] a);
      ...
      begin
         ...
         // bounded value (s <= 128): constant modulo = small LUT function
         return CLM'(s % SRW'(NB));
      end
   endfunction

   // one-hot variant: reduction and bank compare fused per output bit
   function automatic logic [NB-1:0] mod17_oh(input logic [31:0] a);
      logic [CLM-1:0] r;
      logic [NB-1:0]  oh;
      begin
         r = mod17(a);
   ...
   function automatic logic [CLM-1:0] mod17_add(input logic [CLM-1:0] a, b);
      logic [CLM:0] s;
      begin
         s = {1'b0, a} + {1'b0, b};
         if (s >= (CLM + 1)'(NB)) begin
   ...
   // (i * r) mod 17 for constant i, r < 17: pure 5-bit LUT functions
   function automatic logic [CLM-1:0] mod17_muli(input logic [CLM-1:0] r,
   ...
   assign smod  = mod17({{(32 - STRIDE_W){1'b0}}, stride});
   assign scorr = stride[STRIDE_W-1] ? mod17_add(smod, CLM'(NB - CM))
                                     : smod;
   ...
   assign bank0_oh   = mod17_oh({{(32 - AW){1'b0}}, addr});
   assign bank[0]    = f_enc(bank0_oh);

   generate
      for (genvar i = 1; i < NL; i = i + 1) begin: gen_lane
         // i*stride as a constant shift-add (i <= 15: at most 3 adds)
         ...
         assign bank[i]    = mod17_add(bank[0], mod17_muli(scorr, i));
      end
   endgenerate

   //----------------------------------------------------------------
   // Out-of-range: sign | (top bits >= 17) -- no carry chain after
   ...
   // selects use first-match among lanes 0..14 with lane 15 as default
   //----------------------------------------------------------------
   ...
   assign bankb_oh = mod17_oh({{(32 - AW){1'b0}}, addrb});
   ...
   //----------------------------------------------------------------
   // Return path: per-lane 17:1 bank mux, select = registered bank id
