...
// Self-checking testbench for parmem11_8 (11 banks, 8 lanes).
// Phases:
//   1. bijection: single-lane fill, single-lane read back
//   2. group read, signed strides with stride % 11 != 0: no conflict,
//      every active lane correct
//   3. stride % 11 == 0 (including 0): conflict, lane 0 wins
...
module parmem11_8_tb();

   parameter DEPTH    = 4;
   parameter WIDTH    = 32;
   parameter STRIDE_W = 6;

   localparam NB_BANKS = 11;
   localparam NB_LANES = 8;
   localparam AW       = DEPTH + 4;
   ...
   parmem11_8 #(.DEPTH(DEPTH), .WIDTH(WIDTH), .STRIDE_W(STRIDE_W),
                .OUTREGA(0), .OUTREGB(0))
   parmem11_8_inst (.clka(clka), .en(en), .wen(wen), .lane_en(lane_en),
   ...
   parmem11_8_adr_inst (.clka(clka), .en(en_p), .wen(wen_p),
   ...
   //----------------------------------------------------------------
   initial begin
      $dumpfile("parmem11_8_tb.vcd");
      $dumpvars(0, parmem11_8_tb);
   end
   ...
   initial begin
      ...
      //--- Summary ------------------------------------------------------
      if (errors == 0) begin
         $display("parmem11_8_tb: ALL TESTS PASSED (%0d words)", WORDS);
      end
      else begin
         $display("parmem11_8_tb: FAILED with %0d error(s)", errors);
      end
      $finish;
   end


endmodule // parmem11_8_tb
