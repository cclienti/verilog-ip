...
module rmii_eth_tcp_endpoint #(
    ...
    parameter int LOG2_TCP_RX_DEPTH  = 11, // TCP receive buffer in bytes, log2
    parameter int LOG2_TCP_TX_DEPTH  = 11, // TCP transmit ring in bytes, log2
    parameter int TCP_MSS            = 1460,
    parameter int TCP_RTO_CLOCKS     = 10000000,
    parameter int TCP_MAX_RETRIES    = 8,
    parameter longint TCP_IDLE_CLOCKS = 64'd30000000000
)(
    ...
    input logic [15:0]  listen_port,  // TCP passive-open port

    // TCP connection status, valid while connected
    ...
    logic [15:0] ipd_tdata;     // demux outputs, ICMP low byte, TCP high
    ...
    #(
        .NB_PROTOCOLS (2),
        .PROTOCOLS    ({8'h06, 8'h01})   // TCP on select 1, ICMP on 0
    )
    ...
    #(
        .LOG2_RX_DEPTH (LOG2_TCP_RX_DEPTH),
        .LOG2_TX_DEPTH (LOG2_TCP_TX_DEPTH),
        .MSS           (TCP_MSS),
        .RTO_CLOCKS    (TCP_RTO_CLOCKS),
        .MAX_RETRIES   (TCP_MAX_RETRIES),
        .IDLE_CLOCKS   (TCP_IDLE_CLOCKS)
    )
    axi_stream_tcp_socket_inst
    (
        .clock        (clock),
        .sreset       (sreset),
        .local_mac    (local_mac),
        ...
        .s_length     (ipp_length),
        .s_src_mac    (ethp_src_mac),
        .m_axi_tdata  (tcp_tdata),
        .m_axi_tuser  (tcp_tuser),
    ...
    (
        .clock        (clock),
        .sreset       (sreset),
        .s_axi_tdata  ({tcp_tdata, icmp_tdata, arp_tdata}),
        .s_axi_tuser  ({tcp_tuser, icmp_tuser, arp_tuser}),
        .s_axi_tvalid ({tcp_tvalid, icmp_tvalid, arp_tvalid}),
        .s_axi_tlast  ({tcp_tlast, icmp_tlast, arp_tlast}),
        .s_axi_tready ({tcp_tready, icmp_tready, arp_tready}),
        .s_info       (3'b000),
