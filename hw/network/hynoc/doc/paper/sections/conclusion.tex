...
Verilator co-simulation on a $4\times4$ mesh measures a deterministic per-hop
latency: return-path latency satisfies $L=12H+P-1$ (dual-clock) and
$L=9H+P-1$ (single-clock), the $3$-cycle-per-hop difference being exactly the
depth of the dual-clock FIFO's gray-code synchroniser, which single-clock
operation omits.  A
...
with zero cross-quadrant link traffic, confirming the deterministic
