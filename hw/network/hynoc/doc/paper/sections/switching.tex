...
The end-to-end latency $L$ decomposes into two additive terms.

\emph{Setup phase.} The routing flit propagates hop by hop; at each hop the
ingress FSM reads the flit from the FIFO, asserts a request, and waits for a
grant. Per-hop setup latency is:
\begin{equation}
  T_\text{hop} = T_\text{FIFO} + 1 + T_\text{PRRA}
  \label{eq:thop}
\end{equation}
where $T_\text{FIFO}$ is the FIFO read latency (1~cycle in single-clock mode;
$\approx$3~cycles in dual-clock mode due to the gray-code CDC synchronisation
pipeline~\cite{cummings2002}), the $+1$ accounts for the FSM decode cycle, and
$T_\text{PRRA} \in \{1, 2\}$ is the arbiter grant latency controlled by the
\texttt{PIPELINE} parameter.

\emph{Transfer phase.} Once the path is established, $P$ payload flits are
pipelined at one flit per hop: the first flit arrives at the destination after
$H$ cycles, and the last after $H + P - 1$ cycles.

The total latency is therefore:
\begin{equation}
  L = H \cdot (T_\text{FIFO} + 2 + T_\text{PRRA}) + P - 1
  \label{eq:latency}
\end{equation}

In the minimum configuration (single-clock, \texttt{PIPELINE=0}):
$L = 4H + P - 1$.
In the maximum configuration (dual-clock, \texttt{PIPELINE=1}):
$L = 7H + P - 1$.
Both bounds are tight and deterministic: $L$ depends only on the
statically-known route length $H$ and packet size $P$, with no
congestion-dependent term once the path is open.
