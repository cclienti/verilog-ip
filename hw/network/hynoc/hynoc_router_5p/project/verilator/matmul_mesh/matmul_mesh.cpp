...
static int opp(int p) { const int t[5]={0,3,4,1,2}; return t[p]; }
...
static uint64_t dflit(uint32_t v, bool last)
{
    return ((uint64_t)(last ? 1u : 0u) << 32) | v;
}

// ── Inter-router link state ───────────────────────────────────────────────────
// Signals are latched after rising edge, applied before next rising edge.

struct Link { uint64_t data = 0; uint8_t write = 0, level = 0; };
...
static void capture_links()
{
    for (int r = 0; r < R; ++r)
        for (int c = 0; c < C-1; ++c) {
            auto& a = *routers[r][c]; auto& b = *routers[r][c+1];
            EW[r][c] = { a.port1_egress_data, a.port1_egress_write,
                         b.port3_ingress_fifo_level };
            WE[r][c] = { b.port3_egress_data, b.port3_egress_write,
                         a.port1_ingress_fifo_level };
        }
    for (int r = 0; r < R-1; ++r)
        for (int c = 0; c < C; ++c) {
            auto& a = *routers[r][c]; auto& b = *routers[r+1][c];
            SN[r][c] = { a.port2_egress_data, a.port2_egress_write,
                         b.port4_ingress_fifo_level };
            NS[r][c] = { b.port4_egress_data, b.port4_egress_write,
                         a.port2_ingress_fifo_level };
        }
}
...
static void apply_links()
{
    ...
    for (int r = 0; r < R; ++r)
        for (int c = 0; c < C; ++c) {
            auto& d = *routers[r][c];
            d.port1_ingress_write=0; d.port1_ingress_data=0; d.port1_egress_fifo_level=0;
            d.port2_ingress_write=0; d.port2_ingress_data=0; d.port2_egress_fifo_level=0;
            d.port3_ingress_write=0; d.port3_ingress_data=0; d.port3_egress_fifo_level=0;
            d.port4_ingress_write=0; d.port4_ingress_data=0; d.port4_egress_fifo_level=0;
...
static void clk_edge(uint8_t val)
{
    for (int r = 0; r < R; ++r)
        for (int c = 0; c < C; ++c) {
            auto& d = *routers[r][c];
            d.router_clk = val;
            d.port0_ingress_clk = d.port1_ingress_clk = d.port2_ingress_clk =
            d.port3_ingress_clk = d.port4_ingress_clk = val;
            d.eval();
        }
}
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < N; ++i)
        for (int k = 0; k < K; ++k) A[i][k] = (uint32_t)((i+1)*(k+1));
    for (int k = 0; k < K; ++k) x[k] = (uint32_t)(k+1);
    for (int i = 0; i < N; ++i) {
        y_ref[i] = 0;
        for (int k = 0; k < K; ++k) y_ref[i] += A[i][k] * x[k];
    }
    ...
    for (int r = 0; r < R; ++r)
        for (int c = 0; c < C; ++c) {
            ...
            d.port0_ingress_write = 0; d.port0_ingress_data = 0;
            d.port0_egress_fifo_level = 0;
        }
    for (int i = 0; i < 20; ++i) { clk_edge(1); clk_edge(0); }
    ...
    for (int i = 0; i < 5; ++i) { apply_links(); clk_edge(1); capture_links(); clk_edge(0); }
    ...
    uint32_t y[N]   = {};
    bool     y_rx[N]= {};
    y[0] = y_ref[0]; y_rx[0] = true;   // master computes y[0] locally
    int results = 1;
    ...
    for (int cyc = 0; cyc < MAX_CYCLES && results < N; ++cyc) {
                ...
                if (!tx[r][c].empty() && !d.port0_ingress_full) {
                    ...
                    if (r == 0 && c == 0) {
                        if (tx00_pops % FWDPKT == 0) {
                            ...
                            if (idx < N) fwd_tx_cycle[idx/C][idx%C] = cycles;
                        }
                        ++tx00_pops;
        ...
        for (int r = 0; r < R; ++r)
            for (int c = 0; c < C; ++c) {
                auto& d = *routers[r][c];
                if (!d.port0_egress_write) continue;
                ...
                if (r == 0 && c == 0) {
                    ...
                    if (!in_result) {
                        ptag = val; in_result = true;
                    } else {
                        assert(stop);
                        int i = (int)ptag;
                        if (i > 0 && i < N && !y_rx[i]) {
                            y[i] = val; y_rx[i] = true; ++results;
    ...
    for (int r = 0; r < R; ++r)
        for (int c = 0; c < C; ++c) { routers[r][c]->final(); delete routers[r][c]; }
    delete ctx;
    ...
    printf("%-8s  %4s  %8s  %8s  %8s  %10s  %8s\n",
           "Node", "H", "TX-cyc", "RX-cyc", "Latency", "Formula", "Delta");
    for (int i = 1; i < N; ++i) {
        int r = i / C, c = i % C;
        int H   = r + c + 1;
        ...
        printf("(%d,%d)    %4d  %8llu  %8llu  %8d  %10d  %8d\n",
               r, c, H,
               (unsigned long long)fwd_tx_cycle[r][c],
               (unsigned long long)fwd_rx_cycle[r][c],
               lat, formula, lat - formula);
    }
    ...
    for (int i = 1; i < N; ++i) {
        ...
        printf("(%d,%d)    %4d  %8llu  %8llu  %8d  %10d  %8d\n",
               r, c, H,
               (unsigned long long)ret_tx_cycle[r][c],
               (unsigned long long)ret_rx_cycle[r][c],
               lat, formula, lat - formula);
    }
    ...
    for (int r = 0; r < R; ++r)
        for (int c = 0; c < C-1; ++c) {
            printf("E→W        [%d][%d→%d]  %8llu  %5.1f%%\n",
                   r, c, c+1, (unsigned long long)link_ew[r][c],
                   100.0*link_ew[r][c]/cycles);
            printf("W→E        [%d][%d←%d]  %8llu  %5.1f%%\n",
                   r, c, c+1, (unsigned long long)link_we[r][c],
                   100.0*link_we[r][c]/cycles);
    ...
    for (int r = 0; r < R-1; ++r)
        for (int c = 0; c < C; ++c) {
            printf("S→N        [%d→%d][%d]  %8llu  %5.1f%%\n",
                   r, r+1, c, (unsigned long long)link_sn[r][c],
                   100.0*link_sn[r][c]/cycles);
            printf("N→S        [%d←%d][%d]  %8llu  %5.1f%%\n",
                   r, r+1, c, (unsigned long long)link_ns[r][c],
                   100.0*link_ns[r][c]/cycles);
