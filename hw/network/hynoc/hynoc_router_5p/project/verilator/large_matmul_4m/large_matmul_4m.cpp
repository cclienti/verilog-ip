...
static float fp16_to_float(uint16_t h)
{
    ...
    if (exp == 0)
        ...
        b = ((uint32_t)sign << 31) | ((uint32_t)(exp - 15u + 127u) << 23)
          | ((uint32_t)mant << 13);
...
{
    uint32_t b = (uint32_t)bf << 16; float f; memcpy(&f, &b, 4); return f;
}
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < D_OUT; ++i)
        for (int k = 0; k < D_IN; ++k) W[i][k] = (int8_t)((i%8+1)*(k%4+1));
    for (int k = 0; k < D_IN; ++k) x[k] = (float)(k%4+1);
    ...
    for (int m = 0; m < NM; ++m) {
        for (int t = 0; t < ROUNDS; ++t) {
            ...
            for (int k = 0; k < D_IN; ++k) y[row] += (float)W[row][k] * x[k];
            y_rx[row] = true;
        }
    }
        ...
        for (int r = 0; r < R; ++r)
            for (int c = 0; c < C; ++c) {
                ...
                if (node_is_master[r][c]) {
                    ...
                    if (!in_result[m]) {
                        ptag[m] = val; in_result[m] = true;
                    } else {
                        assert(stop);
                        int row = (int)ptag[m];
                        if (row >= 0 && row < D_OUT && !y_rx[row]) {
                            float res; memcpy(&res, &val, 4);
                            y[row] = res; y_rx[row] = true; ++results;
                        }
                        in_result[m] = false;
                    }
                } else {
                    ...
                    if (stop) {
                        ...
                        float dot = 0.0f;
                        for (int b = 0; b < BLOCKS; ++b) {
                            float scale = fp16_to_float(
                                (uint16_t)(wpkt[r][c][1 + b*9] & 0xFFFF));
                            float partial = 0.0f;
                            for (int j = 0; j < 8; ++j) {
                        ...
                        uint32_t rbits; memcpy(&rbits, &dot, 4);
    ...
    printf("Correctness: %d/%d PASS%s\n\n",
           pass_count, D_OUT, fail_count ? " — SOME FAILURES" : " — ALL PASS");
